...
# Get Windows Server from Active Directory - you can amend this to gather all computers or create your own filter. I needed to be an Enterprise Admin.
# I sorted it whilst testing as I had put test files on d: and e: for some servers and it meant these came up first. It is not necessray in operation.

$Domains = (Get-Adforest).domains
foreach($Domain in $Domains){
  $Computers = get-adcomputer -filter 'OperatingSystem -like "*Windows Server*"' -server $Domain | select DNSHostName | sort -property DNSHostName
  }
